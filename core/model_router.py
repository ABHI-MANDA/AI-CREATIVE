...
def model_candidates(creative_type, forced_id=None):
    """Ordered list of usable models to try: forced → hosted → local."""
    options = available_models(creative_type)
    usable = [m for m in options if m.get("usable")]
    if not usable:
        return []
    if forced_id:
        forced = next((m for m in usable if m["id"] == forced_id), None)
        if forced:
            others = [m for m in usable if m["id"] != forced_id]
            return [forced] + others
    hosted = [m for m in usable if m["kind"] == "hosted"]
    local = [m for m in usable if m["kind"] == "local"]
    return hosted + local
