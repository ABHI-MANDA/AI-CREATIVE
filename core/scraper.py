...
import hashlib, json, mimetypes, re, shutil, socket, ipaddress
...
def _download(url, dest, max_mb=None):
    ...
    r = requests.get(url, headers=HEADERS, timeout=config.SCRAPE_TIMEOUT, stream=True, allow_redirects=True)
    r.raise_for_status()
...
def scrape_url(url, pid):
    ...
    try:
        _validate_public_url(url)
        r = requests.get(url, headers=HEADERS, timeout=config.SCRAPE_TIMEOUT, allow_redirects=True)
        r.raise_for_status()
        final_url = r.url
        raw_html = r.text
        ...
        assets = []
        # Download images
        for i, u in enumerate(clean_images):
            try:
                ...
                assets.append({'type': 'image', 'url': u, 'error': str(e)})

        # Download videos
        for i, u in enumerate(clean_videos):
            try:
                ...
                assets.append({
                    ...
                    'path': str(p.relative_to(config.ROOT)),
                    'signals': vs,
                    'frame_paths': [str(Path(x).relative_to(config.ROOT)) for x in vs.get('frame_paths', [])]
                })
            except Exception as e:
                assets.append({'type': 'video', 'url': u, 'error': str(e)})
