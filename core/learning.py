...
def record_review(project_id,ctype,prompt,output_filename,approved,notes='',rating=None):
    d=load_json(config.REVIEW_STORE,{'reviews':[]})
    ...
    d.setdefault('reviews',[]).append(r); d['reviews']=d['reviews'][-config.MAX_REVIEW_MEMORY*3:]; save_json(config.REVIEW_STORE,d); _memory(r); return r
def _memory(r):
    m=load_json(config.MEMORY_STORE,{'lessons':[],'stats':{'reviews':0,'approved':0,'rejected':0}})
    ...
    save_json(config.MEMORY_STORE,m)
def get_context(ctype,limit=8):
    m=load_json(config.MEMORY_STORE,{'lessons':[]}); rows=[x for x in m.get('lessons',[]) if x.get('creative_type') in (ctype,'all')][-limit:]
    if not rows:return ''
    return 'LEARNED REVIEW LESSONS:\n'+'\n'.join(f"- [{'APPROVED' if x.get('approved') else 'CORRECTION'}] {x.get('lesson','')}" for x in rows)
def summary(): return load_json(config.MEMORY_STORE,{'lessons':[],'stats':{'reviews':0,'approved':0,'rejected':0}})
